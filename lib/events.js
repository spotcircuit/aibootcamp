 ...
 * @param {boolean} includeArchived - Whether to include archived events (default: false)
...
export async function getAllEvents(includeArchived = false) {
  try {
    ...
    // Only include non-archived events by default
    if (!includeArchived) {
 ...
 * @param {boolean} includeArchived - Whether to include archived events (default: true)
 * @returns {Promise<Object>} - A promise that resolves to the event object
 */
export async function getEventById(id, includeArchived = true) {
  try {
    let query = supabase
      .from('events')
      .select('*')
    ...
    // Only include non-archived events if specified
    if (!includeArchived) {
      query = query.eq('archived', false);
    }
