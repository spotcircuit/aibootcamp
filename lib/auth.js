...
export const signIn = async (email, password) => {
  try {
    ...
    try {
      ...
      // Handle specific Supabase errors that might be thrown
      ...
      return {
        ...
        error: {
          message: supabaseError.message || 'Authentication failed. Please try again later.'
        }
      };
    }
    ...
    if (response.error) {
      if (response.error.message === 'Invalid login credentials') {
        response.error.message = 'The email or password you entered is incorrect. Please try again.';
      } else if (response.error.message === 'Email not confirmed') {
        response.error.message = 'Your email has not been confirmed. Please check your inbox for a confirmation email or run the auto_confirm_users.sql script to bypass email confirmation.';
      } else if (response.error.message.includes('Unexpected failure')) {
        response.error.message = 'The authentication service is currently experiencing issues. Please try again later.';
      }
    }
