...
export const signUp = async (email, password, metadata = {}) => {
  try {
    ...
    // Send welcome email if signup was successful and we have a user
    if (response.data?.user) {
      try {
        console.log('Attempting to send welcome email for new user:', response.data.user.id);
        
        // Use fetch to call our welcome email API
        const emailResponse = await fetch('/api/sendWelcomeEmail', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            userId: response.data.user.id,
            email: email,
            name: metadata.name || email.split('@')[0] // Use name from metadata or fallback to part of email
          })
        });
